...
where
    ...
    and (SELECT value.int_value FROM unnest(event_params) WHERE event_name in ('search') and key = 'is_one_item') = 1
    and traffic_source.source in ('(direct)', '(not set)')
