...
sum(select_item) klikniecia_w_produkt,
...
select
    ...
    x.item_list_name,
    count( distinct (SELECT value.string_value FROM unnest(event_params) WHERE key = 'item_id') ) select_item
...
from reco_view_item_list v1 left join reco_select_item v2 on v1.true_date=v2.true_date and v1.device=v2.device and v1.user_pseudo_id=v2.user_pseudo_id and v1.item_list_name=v2.item_list_name
left join reco_purchase v3 on v2.true_date=v3.true_date and v2.device=v3.device and v2.user_pseudo_id=v3.user_pseudo_id and v2.item_id=v3.item_id
